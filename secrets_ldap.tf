...
resource "vault_ldap_secret_backend_dynamic_role" "this" {
  ...
  username_template = "{{ printf \"%s-adm-%s-%s\" (.DisplayName | lowercase | replace \"ldap-\" \"\" | replace \"-\" \"\" | replace \"_\" \"\" | truncate 5) (.RoleName | lowercase | replace \"-\" \"\" | replace \"_\" \"\" | truncate 4) (timestamp \"0106\") | truncate 20 }}"
  max_ttl = 28800
}